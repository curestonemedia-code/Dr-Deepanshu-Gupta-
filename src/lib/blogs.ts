...
// This dataset is shared with thecurestone.com — one Studio, one taxonomy.
// Categories, tags and authors are common to both; only blogPost documents
// are site-specific, via `siteId`. Every query below scopes to it so the two
// sites' posts never leak into each other's blog index, category counts, or
// related-posts lists.
//
// No coalesce() here, unlike the Cure Stone side: a missing `siteId`
// resolves to Cure Stone, so this site matches the explicit value only.
const SITE_MATCH = `siteId == "dr-deepanshu"`;
...
export type PortableTextBlock = {
   ...
   *  Cure Stone repo, shared). Powers VideoObject JSON-LD on the blog post
   *  page; both are optional in Sanity. */
  uploadDate?: string;
  duration?: string;
  rows?: Array<{
    _key: string;
